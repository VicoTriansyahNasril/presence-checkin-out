...
const editAdminInformationSchema = Yup.object().shape({
  first_name: Yup.string()
    .min(2, messages.first_name.minLength)
    .max(50, messages.first_name.maxLength)
    .required(messages.first_name.required),
...
const changePasswordSchema = Yup.object().shape({
  ...
  retype_new_password: Yup.string()
    .oneOf([Yup.ref('new_password'), null], messages.confirmPassword.notMatch)
    .required(messages.confirmPassword.required),
});

export { validationSchema, editAdminInformationSchema, changePasswordSchema };
